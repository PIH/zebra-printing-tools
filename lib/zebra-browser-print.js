 ...
 *     device.sendFile(blob): Promise<string>             POST /write (multipart)
 *
 ...
 *   - The `Zebra.Printer` wrapper's queue/Promise dual API. The
 *     proprietary SDK lets callers pass an onSuccess/onError pair OR
 *     await the returned Promise; this client is Promise-only.
 *
...
;(function () {
  ...
  //
  // Content-Type for JSON bodies is `text/plain;charset=UTF-8`, NOT
  // `application/json`. Reason: Zebra's localhost helper sets CORS
  // headers but doesn't pre-flight cleanly with `application/json`
  // (which the browser classifies as a non-simple request and OPTIONS-
  // preflights before every POST). Each preflight adds a full helper
  // round-trip to /write and /read, doubling status/info latency on
  // Windows + Browser Print to the point where the page-level
  // STATUS_TIMEOUT_MS races trip. The proprietary SDK uses XHR with no
  // explicit Content-Type — its string body defaults to text/plain,
  // which is a simple-request type and skips the preflight. We mirror
  // that. The helper's body parser ignores Content-Type and just JSON-
  // decodes whatever it receives; the lie is harmless.
  ...
  class Device {
    ...
    // Send a query command and drain the reply. Two termination modes:
    //
    //   - opts.until = '\x03' (or any string): exit as soon as the
    //     accumulated buffer contains that terminator. Used for ZPL
    //     status (~hs), info (~hi), and config (^XA^HH^XZ) queries
    //     which all end with ETX (0x03). Mirrors the proprietary SDK's
    //     `sendThenReadUntilStringReceived(cmd, '\x03', ...)` for
    //     these query types — saves the trailing drain wait, which on
    //     Windows + Browser Print can otherwise tip the call past
    //     STATUS_TIMEOUT_MS.
    //
    //   - no until: drain-until-quiet — exit after `idleQuietMs` of
    //     empty /read responses with content already buffered. Used
    //     for SGD getvar replies, which some firmwares wrap in STX/ETX
    //     and others don't, so we can't rely on a fixed terminator.
    //
    // Either way, `hardCapMs` is the fallback for "printer never
    // replied at all" — caller sees an empty string and treats it as
    // a timeout.
    //
    // Defaults: poll every 100 ms, idle quiet 150 ms, hard cap 1500 ms.
    // Tune via opts for slow links / slow commands.
    ...
    async _sendAndReadImpl(data, opts) {
      ...
      while (true) {
        ...
        if (chunk) {
          buffer += chunk;
          lastDataAt = Date.now();
          // Fast-exit: terminator landed in the buffer. Saves the
          // trailing idleQuietMs wait — critical when the page is
          // racing the call against its own per-query timer.
          if (until && buffer.indexOf(until) >= 0) break;
        }
        const now = Date.now();
        // Quiet drain (no until set): we have data and nothing new for
        // a moment → done.
        if (!until && buffer && (now - lastDataAt) > idleQuietMs) break;
        // Hard cap regardless — covers the "printer never replied"
        // case (offline / wrong command / wrong query).
    ...
    // Send a Blob to the printer as raw bytes via POST /write (multipart).
    // Bypasses helper-side conversion entirely — the printer firmware
    // sees the bytes as-is. Useful for PDF Direct (firmware rasterises
    // a PDF when apl.enable = "pdf") and other firmware-side formats.
    //
    // Wire: multipart/form-data POST /write with two parts:
    //   - "json" = '{"device": <deviceRecord>}'    (no options/action/data)
    //   - "blob" = raw bytes
    //
    // Distinct from /convert (used by convertAndSendFile), which carries
    // options.fromFormat / options.action and runs the helper's licensed
    // converter. See spec docs/superpowers/specs/2026-05-11-pdf-direct-
    // testing-design.md §3 for the wire-shape derivation.
    async sendFile(blob) {
      return this._enqueue(() => this._sendFileImpl(blob));
    }

    async _sendFileImpl(blob) {
      if (typeof Blob === 'undefined' || !(blob instanceof Blob)) {
        throw new TypeError(
          'Device.sendFile() expects a Blob (File extends Blob, so File works too)'
        );
      }
      const fd = new FormData();
      fd.append('json', JSON.stringify({ device: this._json }));
      fd.append('blob', blob);
      // Do NOT set Content-Type here — fetch auto-generates
      // multipart/form-data; boundary=... when body is FormData.
      // Setting it manually strips the boundary and breaks parsing on
      // both Zebra's helper and our shim.
      const r = await fetch(HELPER_BASE + '/write', { method: 'POST', body: fd });
      const text = await r.text();
      if (!r.ok) {
        throw new Error(
          'Browser Print helper POST /write returned ' + r.status + ': ' + text
        );
      }
      return text;   // helper returns empty string on success
    }
  }
  ...
  class Printer {
    ...
    async getStatus() {
      // ~hs reply is three STX/ETX frames flushed back-to-back. The
      // proprietary SDK terminates on the first ETX (and relies on
      // helper buffering all three frames into one /read response by
      // then). We do the same.
    ...
    async getConfiguration() {
      // ^HH dump is ~3-4 KB of multi-line text terminated with ETX.
      // First byte can take ~1.5 s on Windows + Browser Print to
      // arrive, so a generous hardCapMs is the offline fallback;
      // until: '\x03' is the happy-path exit.
      const raw = await this.device.sendAndRead('^XA^HH^XZ', { until: '\x03', hardCapMs: 4000 });
      return parseConfigResponse(raw);
    }

    async getInfo() {
      // ~hi reply is one STX/ETX-framed CSV record.

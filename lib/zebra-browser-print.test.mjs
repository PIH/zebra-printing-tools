...
await test('device.send(zpl) → POST /write with {device, data}', async () => {
  ...
  // text/plain (a "simple request" CT) — see CORS-preflight test below
  // for why this isn't application/json.
  assertEqual(write.headers['Content-Type'], 'text/plain;charset=UTF-8');
...
// Phase 3: sendFile (PDF Direct — raw blob to /write, firmware rasterizes)
// See spec: docs/superpowers/specs/2026-05-11-pdf-direct-testing-design.md §4a.

console.log('Phase 2 — sendFile');

await test('sendFile posts to /write (not /convert)', async () => {
  let writeCall = null;
  mockFetch((call) => {
    if (call.url.endsWith('/default?type=printer')) {
      return { status: 200, body: JSON.stringify({ uid: 'u', name: 'A', deviceType: 'printer' }) };
    ...
    if (call.url.endsWith('/convert')) {
      throw new Error('sendFile must NOT hit /convert');
    }
    return { status: 404, body: '' };
  });
  const d = await ZBP.getDefaultDevice('printer');
  await d.sendFile(new Blob(['%PDF-1.4 fake'], { type: 'application/pdf' }));
  assert(writeCall, 'helper /write must be called');
  assertEqual(writeCall.method, 'POST');
});

await test('sendFile body is FormData with json+blob parts (no options/action)', async () => {
  let writeCall = null;
  mockFetch((call) => {
    if (call.url.endsWith('/default?type=printer')) {
      return { status: 200, body: JSON.stringify({ uid: 'u', name: 'A', deviceType: 'printer' }) };
    }
    if (call.url.endsWith('/write')) { writeCall = call; return { status: 200, body: '' }; }
    return { status: 404 };
  });
  const d = await ZBP.getDefaultDevice('printer');
  const blob = new Blob(['%PDF-1.4 fake'], { type: 'application/pdf' });
  await d.sendFile(blob);
  assert(writeCall.body instanceof FormData, 'body should be FormData');
  const json = JSON.parse(writeCall.body.get('json'));
  // Critical: NO options / action / data — distinguishes /write multipart
  // (sendFile) from /convert multipart (convertAndSendFile).
  assertEqual(Object.keys(json).sort(), ['device'], 'json envelope has only `device`');
  assertEqual(json.device.uid, 'u');
  const blobOut = writeCall.body.get('blob');
  assert(blobOut instanceof Blob, 'blob part should be a Blob');
  assertEqual(blobOut.type, 'application/pdf');
});

await test('sendFile rejects non-Blob argument', async () => {
  mockFetch(() => ({ status: 200, body: JSON.stringify({ uid: 'u', deviceType: 'printer' }) }));
  const d = await ZBP.getDefaultDevice('printer');
  let threw = false;
  try { await d.sendFile('not a blob'); } catch (e) { threw = true; assert(/Blob/.test(e.message), 'message mentions Blob'); }
  assert(threw, 'should reject non-Blob');
  threw = false;
  try { await d.sendFile(undefined); } catch (e) { threw = true; assert(/Blob/.test(e.message), 'message mentions Blob'); }
  assert(threw, 'should reject undefined');
});

await test('sendFile surfaces helper error in message on non-2xx', async () => {
  mockFetch((call) => {
    if (call.url.endsWith('/default?type=printer')) {
      return { status: 200, body: JSON.stringify({ uid: 'u', deviceType: 'printer' }) };
    }
    if (call.url.endsWith('/write')) {
      return { status: 500, body: 'helper says nope' };
    }
    return { status: 404 };
  });
  const d = await ZBP.getDefaultDevice('printer');
  let threw = null;
  try { await d.sendFile(new Blob(['x'], { type: 'application/pdf' })); }
  catch (e) { threw = e; }
  assert(threw, 'should reject on 500');
  assert(/500/.test(threw.message), 'message includes status');
  assert(/helper says nope/.test(threw.message), 'message includes helper body');
});

await test('sendFile participates in per-Device queue (does not race a follow-up read)', async () => {
  // Verifies sendFile goes through the same _enqueue path as send/read/sendAndRead/
  // convertAndSendFile. Without this, a sendFile followed by getStatus could
  // interleave wire bytes — exactly the bug entry 42 fixed for the other ops.
  const order = [];
  ...
  const d = await ZBP.getDefaultDevice('printer');
  await Promise.all([
    d.sendFile(new Blob(['x'], { type: 'application/pdf' })),
    d.read(),
  ]);
  // The write must complete before the read starts. Without the queue,
  // the read's POST /read would fire concurrently with the write — order
  // would either be ['read', 'write'] or interleaved chunks.
  assertEqual(order, ['write', 'read'], 'sendFile must complete before read');
});
...
await test('JSON POSTs use a simple Content-Type to skip CORS preflight', async () => {
  // CORS rules: a POST with `Content-Type: application/json` is a
  // non-simple request and the browser fires OPTIONS before every
  // /write and /read, doubling helper round-trips. Browser Print on
  // Windows is ~750 ms per round-trip; doubling that pushes status
  // past STATUS_TIMEOUT_MS in the page. The proprietary SDK uses XHR
  // with no explicit Content-Type (defaults to text/plain); we must
  // do the same. Helper's body parser ignores the declared type.
  const seenTypes = [];
  mockFetch((call) => {
    const ct = (call.headers && (call.headers['Content-Type'] || call.headers['content-type'])) || null;
    if (call.method === 'POST') seenTypes.push(ct);
    if (call.url.endsWith('/default?type=printer')) {
      return { status: 200, body: JSON.stringify({ uid: 'u', deviceType: 'printer' }) };
    }
    if (call.url.endsWith('/write') || call.url.endsWith('/read')) {
      return { status: 200, body: '' };
    }
    return { status: 404 };
  });
  ...
  await d.send('hello');
  await d.read();
  // Each /write and /read POST must declare a SIMPLE Content-Type.
  // Allowed: omitted, text/plain (with charset), application/x-www-form-urlencoded,
  // multipart/form-data. NOT allowed: application/json (preflight trigger).
  const SIMPLE_RE = /^(text\/plain|application\/x-www-form-urlencoded|multipart\/form-data)\b/i;
  for (const ct of seenTypes) {
    if (ct == null) continue;  // omitted is fine
    assert(SIMPLE_RE.test(ct),
      'POST Content-Type ' + JSON.stringify(ct) + ' triggers CORS preflight (must be simple)');
    assert(!/^application\/json/i.test(ct),
      'POST Content-Type ' + JSON.stringify(ct) + ' is application/json — triggers preflight');
  }
});

await test('sendAndRead exits on `until` terminator without waiting for idleQuiet', async () => {
  // Mirrors the proprietary SDK's sendThenReadUntilStringReceived for
  // status/info/config — once the buffer contains the ETX terminator,
  // we're done. Without this, a slow Windows+Browser Print helper
  // blocks the call past STATUS_TIMEOUT_MS waiting for the trailing
  // idleQuietMs even after the data has fully arrived.
  let readCalls = 0;
  mockFetch((call) => {
    if (call.url.endsWith('/default?type=printer')) {
      return { status: 200, body: JSON.stringify({ uid: 'u', deviceType: 'printer' }) };
    ...
    if (call.url.endsWith('/read')) {
      readCalls++;
      if (readCalls === 1) {
        // First read returns the full reply with terminator.
        return { status: 200, body: '\x02FRAME1\x03\x02FRAME2\x03\x02FRAME3\x03' };
      }
      return { status: 200, body: '' };
    }
    return { status: 404 };
  ...
  const d = await ZBP.getDefaultDevice('printer');
  const t0 = Date.now();
  // idleQuietMs=300 would normally add ≥300 ms of trailing drain;
  // until exits as soon as ETX is seen, so the call returns near the
  // first /read latency.
  const reply = await d.sendAndRead('~hs', {
    pollIntervalMs: 30, idleQuietMs: 300, hardCapMs: 2000,
    until: '\x03',
  });
  const dt = Date.now() - t0;
  assert(reply.indexOf('FRAME3') >= 0, 'expected all three frames, got: ' + JSON.stringify(reply));
  // Expect well under (poll + first read + idleQuiet) ≈ 30 + 300 = 330 ms.
  // Without `until` we'd wait the full 300 ms idleQuiet after data lands.
  assert(dt < 200, 'sendAndRead with until terminator should exit fast, took ' + dt + ' ms');
  assert(readCalls <= 2, 'expected ≤2 /read calls (first returns data, optional second confirms quiet), got ' + readCalls);
});

await test('sendAndRead without `until` still drains until quiet', async () => {
  // SGD getvar replies aren't always STX/ETX framed, so the SGD path
  // doesn't pass `until` and falls back to drain-until-quiet. Verify
  // that the no-until path still works.
  let readCalls = 0;
  mockFetch((call) => {
    if (call.url.endsWith('/default?type=printer')) {
      return { status: 200, body: JSON.stringify({ uid: 'u', deviceType: 'printer' }) };
    ...
    if (call.url.endsWith('/read')) {
      readCalls++;
      if (readCalls === 1) return { status: 200, body: '"300"' };
      return { status: 200, body: '' };
    }
    return { status: 404 };
  });
  const d = await ZBP.getDefaultDevice('printer');
  const reply = await d.sendAndRead('! U1 getvar "x"\r\n', {
    pollIntervalMs: 20, idleQuietMs: 50, hardCapMs: 1000,
  });
  assertEqual(reply, '"300"');
});

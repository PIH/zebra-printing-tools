...
lib/                                ← MPL-2.0 helper-client library (drop-in replacement for Zebra's bundled SDK)
  zebra-browser-print.js            ← Vanilla-JS client for Browser Print's HTTP API. The page loads this in
                                      place of Zebra's proprietary BrowserPrint-3.x.min.js + BrowserPrint-Zebra-
                                      1.x.min.js. Single file, no dependencies, no build step.
  zebra-browser-print.test.mjs      ← Node test suite (mocked-fetch). Run: `node lib/zebra-browser-print.test.mjs`.
  README.md                         ← scope, usage, and what's deliberately omitted from the lib.
...
LICENSE                             ← MPL-2.0 (Mozilla Public License v2.0). Matches PIH's OpenMRS-module convention.
README.md                           ← this file
```
...
The page used to load Zebra's proprietary `BrowserPrint-3.x.min.js` +
`BrowserPrint-Zebra-1.x.min.js` SDK files; those have been retired in
favour of `lib/zebra-browser-print.js` (MPL-2.0, vanilla-JS, same wire
protocol — see `lib/README.md`). For Zebra's own JSDoc reference,
sample code, newer SDK versions, Browser Print helper installers, the
ZPL programming guide, SGD command reference, and PDF Direct
enablement, consult Zebra directly:
...
[MPL-2.0](LICENSE) — Mozilla Public License v2.0. Chosen to match
PIH's OpenMRS-module convention.

The page's helper-API client (`lib/zebra-browser-print.js`) is also
MPL-2.0. The shim (`utils/browser-print-shim.py`) and everything else
in this repo inherit the same license unless a per-file SPDX header
says otherwise.
